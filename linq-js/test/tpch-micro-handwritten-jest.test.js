...
const handwrittenQueries = [undefined];
for (var i = 1; i <= 7 ; i++) {
  handwrittenQueries[i] = require(`../tpch/micro/handwritten/q${i}`);
}
...
describe("TPC-H Tests Micro Benchmark Queries (dynamic tables)", function () {
  ...
  test("Q1", function() {
    var resultLodAsh = handwrittenQueries[1](rawTables);
    var resultCalcite = API.sqlCalcite(queries[1]);
    expect(resultLodAsh).toBeDeepCloseTo(resultCalcite);
  });

  test("Q2", function() {
    var resultLodAsh = handwrittenQueries[2](rawTables);
    var resultCalcite = API.sqlCalcite(queries[2]);
    expect(resultLodAsh).toBeDeepCloseTo(resultCalcite);
  });

  test("Q3", function() {
    var resultLodAsh = handwrittenQueries[3](rawTables);
    var resultCalcite = API.sqlCalcite(queries[3]);
    expect(resultLodAsh).toBeDeepCloseTo(resultCalcite);
  });

  test("Q4", function() {
    var resultLodAsh = handwrittenQueries[4](rawTables);
    var resultCalcite = API.sqlCalcite(queries[4]);
    expect(resultLodAsh).toBeDeepCloseTo(resultCalcite);
  });

  test("Q5", function() {
    var result = handwrittenQueries[5](rawTables);
    var resultCalcite = API.sqlCalcite(queries[5]);
    expect(result).toBeDeepCloseTo(resultCalcite, 3);
  });

  test("Q6", function() {
    var resultLodAsh = handwrittenQueries[6](rawTables);
    var resultCalcite = API.sqlCalcite(queries[6]);
    expect(resultLodAsh).toBeDeepCloseTo(resultCalcite);
  });

  test("Q7", function() {
    var resultLodAsh = handwrittenQueries[7](rawTables);
