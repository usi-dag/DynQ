...
public class RexRTruffleNodeVisitorFactory extends RexTruffleNodeVisitorFactory {
    ...
    public Pair<RexTruffleNodeVisitor, RexTruffleNodeVisitor> createPair(APISessionManagement session,
        ...
        RexTruffleNodeVisitor leftVisitor = new RexRTruffleNodeVisitor(session, relNodeLeft) {
            @Override
            public RexTruffleNode visitInputRef(RexInputRef rexInputRef) {
                return relNodeLeft.getOutputDataAccessor(rexInputRef.getIndex());
            }
        };
        RexTruffleNodeVisitor rightVisitor = new RexRTruffleNodeVisitor(session, relNodeLeft) {
            @Override
            public RexTruffleNode visitInputRef(RexInputRef rexInputRef) {
