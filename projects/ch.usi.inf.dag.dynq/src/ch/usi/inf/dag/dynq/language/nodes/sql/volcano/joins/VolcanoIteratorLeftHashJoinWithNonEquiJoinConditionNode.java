...
public class VolcanoIteratorLeftHashJoinWithNonEquiJoinConditionNode extends VolcanoIteratorNode {
    ...
    public static VolcanoIteratorLeftHashJoinWithNonEquiJoinConditionNode create(VolcanoIteratorNode buildInput,
                                                                          VolcanoIteratorNode probeInput,
                                                                          RexTruffleNode buildKeyGetter,
                                                                          RexTruffleNode probeKeyGetter,
                                                                          BinaryJoinCondition nonEquiCondition) {
        return new VolcanoIteratorLeftHashJoinWithNonEquiJoinConditionNode(buildInput, probeInput, buildKeyGetter, probeKeyGetter, nonEquiCondition);
    }

    VolcanoIteratorLeftHashJoinWithNonEquiJoinConditionNode(VolcanoIteratorNode buildInput,
                                                                   VolcanoIteratorNode probeInput,
                                                                   RexTruffleNode buildKeyGetter,
                                                                   RexTruffleNode probeKeyGetter,
                                                                   BinaryJoinCondition nonEquiCondition) {
            ...
            if (rows != null) {
                for (int i = 0; i < rows.size(); i++) {
                    ...
                    if(nonEquiCondition.execute(frame, buildRow, row)) {
                        destination.execute(frame, Pair.create(buildRow, row));
                    }
                }
            }
        }
